...
def resize_image(force, extension, force_2, extension_2):
    global image_spring, tk_image_spring
    # Convert the new_width argument to an integer
    new_spring_width = image_spring.width - int(force) + int(force_2) + int(extension*100) + int(extension_2)
...
image_spring = Image.open(r"parallel spring.png")
...
def calc_fkx(): 
    force = float(f_scale.get())
    extension = float(x_scale.get())
    spring_constant = float(k_scale.get())

    calc_f = spring_constant * extension
    global fin_ans_f
    fin_ans_f = str(round(calc_f,2))
    f_ans.config(text=f"F = {fin_ans_f} N")

    calc_k = (2 * force) / extension
    global fin_ans_k
    fin_ans_k = str(abs(round(calc_k, 2)))
    k_ans.config(text = f"k = {fin_ans_k} N/m")

    calc_x = force / (2 * spring_constant)
    global fin_ans_x
    fin_ans_x = str(round(calc_x, 2))
    x_ans.config(text = f"x = {fin_ans_x} N/m")
...
def update_graph():
    ...
    global force, extension, force_2, extension_2
    force = float(f_scale.get())
    extension = float(x_scale.get())
    ...
    if force_2 != 0.01:
        force_1 = k_1 * extension
        if abs(force_1) > 100:
            force_1 = 100 if force_1 > 0 else -100
            
        x = 250 + extension * 2 * 100
        y = 250 - force_2 * 1.5
        ...
        EPE = abs(round(((force_2 * extension) * 0.5), 4))
        ...
        # Calculate area under the line
        EPE = abs(round(((force**2) / k_1 * 0.5), 4))
        area_label.config(text=f"Elastic Potential Energy = {EPE} J")

    else:
        x = 250 + extension * 100 * 2
        y = 250 - force * 1.5
        ...
        # Calculate area under the line
        EPE = abs(round(((force * extension) * 0.5), 4))
        area_label.config(text=f"Elastic Potential Energy = {EPE} J")
...
def scale_func(value):
    ...
    resize_image(force, extension, force_2, extension_2)
...
reset_but = tk.Button(mw, text = "RESET", width = 17, relief = tk.RIDGE, bg = "white", activebackground = 'dark grey', font = ('Arial Black', 11), command = reset)
