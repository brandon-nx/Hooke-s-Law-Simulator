...
image_spring = Image.open(r"series spring.png")
...
def update_graph():
    ...
    if force_2 != 0.01:     
        ...
        EPE = abs(force_2 * extension_1) * 0.5
        ...
        # Calculate area under the line
        EPE = abs(round(((force_1 * extension_2) * 0.5), 4))
        area_label.config(text=f"Elastic Potential Energy = {EPE} J")
        ...
        EPE = abs(round(((force_1 * extension_1) * 0.5), 2))
        area_label.config(text=f"Elastic Potential Energy = {EPE} J")
...
reset_but = tk.Button(mw, text = "RESET", width = 17, relief = tk.RIDGE, bg = "white", activebackground = 'dark grey', font = ('Arial Black', 11), command = f_func)
